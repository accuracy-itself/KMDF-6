...
)
{
	...
	KdPrintEx((DPFLTR_IHVDRIVER_ID, DPFLTR_INFO_LEVEL, "KmdfHelloWorld: 051005 Stankevich DriverEntry\n"));
	...
	else
	{
		...
		if (wcsstr(procName->Buffer, L"UserModeProject.exe"))
		{
			...
			if (NT_SUCCESS(ntstatus)) {
				ntstatus = RtlStringCbPrintfA(bufferToWrite, sizeof(bufferToWrite), "z");
				if (NT_SUCCESS(ntstatus)) {
					...
					if (NT_SUCCESS(ntstatus)) {
						...
						DbgPrint("Process %s opened", "USERMODE");
